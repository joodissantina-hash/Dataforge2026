...
def _build_changing_facts(place: Placer, tokens: List[Token], n: int, _m: int,
    ...
    place(first_pos, key, VALUES[0])
    place(second_pos, key, VALUES[3])
...
def _build_distractor_recall(place: Placer,_tokens: List[Token], n: int, mem_slots: int,
    ...
    for i in range(min(competing, len(slots_free))):
        place(slots_free[i], KEYS[(i + 1) % len(KEYS)], VALUES[(i + 1) % len(VALUES)])
    return VALUES[0], (
        f"{competing} facts about OTHER keys compete for {mem_slots} memory slots. "
...
def _build_noisy_prediction(place: Placer, _tokens: List[Token], n: int, _m: int,
    ...
    return VALUES[0], (
        f"{key} is re-affirmed as '{VALUES[0]}' {len(positions)} times. Every "
...
def _build_delayed_recall(place: Placer, _tokens: List[Token], n: int, _m: int,
    ...
    place(1, key, VALUES[0])
    return VALUES[0], (
        f"{key}='{VALUES[0]}' appears at step 1 and is never mentioned again "
        f"before the query at step {n - 1}."
    )
...
def build_sequence(
    ...
    if add_query:
        ...
        notes.append(
            f"DISTRIBUTION SHIFT: the query asks about {target_key}, which never appears "
...
def run_attention(seq: Sequence, noise: float, seed: int) -> Dict[str, Any]:
    ...
    scores = (keys @ q) / np.sqrt(DIM) + BETA_RECENCY * np.log(np.arange(n) / max(1, n))
...
def _memory_process_token(mem: _SlotMemory, t: Token, seq: Sequence, noise: float,
...
def run_memory(seq: Sequence, mem_slots: int, alpha: float, decay: float,
    ...
    return {
         ...
         "retrieved_key": mem.slot_key[read_slot] if read_slot >= 0 else None,
         "retrieved_slot": read_slot if read_slot >= 0 else None,
         "retrieved_age": int(mem.age[read_slot]) if read_slot >= 0 else 0,
...
class _SynapticState:
    ...
    def read(self, query_key: str) -> np.ndarray:
        return self.sigma @ sparsify(embed(query_key))
...
def _bdh_process_token(state: _SynapticState, t: Token, noise: float,
    ...
    pert = noise * (1.5 if t.is_noisy else 1.0)
    xk = sparsify(embed(t.key)) + pert * rng.standard_normal(DIM)
    xv = sparsify(embed(t.value)) + pert * rng.standard_normal(DIM)
...
def _verdict_memory(correct: bool,res: Dict[str, Any],seq: Sequence,
    ...
    if correct:
        return "success", (
            f"A fixed {res['state_floats']}-float state carried the fact across the gap "
...
def _verdict_bdh(correct: bool, res: Dict[str, Any], seq: Sequence,
    ...
    if correct:
        return "success", (
            ...
            f"keys (top-{K_SPARSE} of {DIM}) keep crosstalk low. State: {res['state_floats']} "
...
def simulate(cfg: Dict[str, Any]) -> Dict[str, Any]:
    seq = build_sequence(
        ...
        seed=cfg["seed"], add_query=cfg["ood_query"], reset_midway=cfg["reset_midway"],
    )
